...
**Gate `output/`, not only `dist/`.** `output/` is the real run — the files that get sent. `dist/` is the deterministic sample build a project may define (one fixed anonymized row); it shares no data with `output/`, so a green `dist/` says nothing about the emails going out. Running the gate with no file argument defaults to `dist/email.html`. A "With Variables" project **must** call the gate per row from its own `generate.js` as it writes — that is what keeps the real run from going out ungated. **The agent ships no `generate.js` and no scaffold for one**, so a folder copy does not inherit that guarantee: if your generator does not call `validate()`, `output/` is ungated no matter what this file says.

It exits non-zero on any blocking failure and writes `VALIDATION_REPORT.md`. A "With Variables" project's own `generate.js` **should** also auto-verify a subset at build time (unresolved placeholders, Gmail blend contract, leaf-`<td>` backgrounds) — again its code, not the agent's. Everything the script lists as `[MANUAL]` — Litmus renders, legal footer, minimum sizes, final copy — stays human.
