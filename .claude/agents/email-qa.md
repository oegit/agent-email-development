...
1. **The shared automated gate** — `validate-email.js` lives at the agent root (`agent-email-development/validate-email.js`), not per-project; every project can run it against its own compiled HTML. A project **may** define a placeholder contract — a `qa-tokens.txt` next to its `template.mjml`. If this one does, pass it via `--tokens` to enforce it; if it does not, the script still runs its other checks and just reports the tokens it finds as informational. Check for the file; do not assume either way.
...
- Read that script's own exported manual-items list (printed at the bottom of `VALIDATION_REPORT.md`) as well. **`PRE_SEND_QA.md` is the checklist of record; `MANUAL_ITEMS` is a short subset the gate reprints for convenience.** Walk all of `PRE_SEND_QA.md`'s `[MANUAL]` rows, and treat a `MANUAL_ITEMS` entry as an already-worded restatement of the row it corresponds to — not as a replacement for the walk. The two lists are different lengths on purpose (7 vs 24 at the time of writing); the earlier wording said to substitute one "where the two overlap", with no mapping and no definition of overlap, which is the only thing standing between a 24-item human pass and a 7-item one (audit 2026-07-26 R2, J15).
...
*Updated 2026-07-26 (round-2 audit, `audit/RECONCILE_AGENT_EMAIL_DEVELOPMENT_20260726_R2.md`) · Step 1 says which list wins — `PRE_SEND_QA.md` is the checklist of record and the script's `MANUAL_ITEMS` is a subset it reprints — because "use it instead of the generic one where the two overlap" was undecidable: 7 entries against 24 rows, no mapping, no definition of overlap, and that qualifier was the only thing standing between a 24-item human pass and a 7-item one (J15). Step 1 also no longer asserts that a specific example project has a `qa-tokens.txt`; it says a project may define one and to check (J16). This file has no rolling `Last updated` version line by design — it records dated notes per event, which `docs-contract.test.js` now states as a declared exemption rather than a silence.*
