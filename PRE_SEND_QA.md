...
>
> They share no data with each other, so **a green `dist/` says nothing about `output/`** — until the 2026-07-26 audit (R09/R24/R36) the gate ran only on `dist/`, and every real send went out ungated. **Closing that hole is the project's obligation, not something a folder copy inherits:** a **"With Variables"** project **must** call the gate per row from its own `generate.js` as it writes, and may wrap `output/*.html` in its own `npm run validate`. The agent ships no `generate.js` and no scaffold for one, so if your generator does not call it, `output/` is ungated whatever this file says. On a **Standard** project neither script exists — run the gate yourself against the compiled file: `node <path-to-agent>/validate-email.js output/*.html`.
