...
├── postprocess-email.test.js     ← the post-compile step's behaviour, pinned against fixture <body> tags
├── package.json                  ← makes the agent a declarable dependency; `npm test` runs all three gates
...
| `docs-contract.test.js`, `validate-email.test.js`, `postprocess-email.test.js`, `package.json` | Maintainer tooling for editing the agent, not for using it. |
